;;; o-journal.el --- create and publish a journal with org-mode
;;
;; Copyright © 2012 Farasha Euker <fravashi@mundusimaginal.is>
;;
;; Author: Farasha Euker <fravashi@mundusimaginal.is>
...
(mapcar (lambda (x) (require (intern (format "o-journal-%s" x)) nil t))
	'("copy-files"))

(defconst o-journal-version "1.2" "o-journal version number")

(defgroup o-journal nil "o-journal customization group"
  :group 'org-export)

(defcustom o-journal-async-opts nil
  "Extra options to be used when compiling with
`org-publish-blog-async'."
  :group 'o-journal
  :type 'list)

(defcustom o-journal-before-publish-hook nil
  "Hook to be run before publishing a blog.
Each hook is a function that could be called with no parameter."
  :group 'o-journal
  :type 'hook)

(defcustom o-journal-after-publish-hook nil
  "Hook to be run after publishing a blog.
Each hook is a function that could be called with no parameter."
  :group 'o-journal
  :type 'hook)

(defcustom o-journal-html-plugins-hook nil
  "Hook to be run before exporting an entry to HTML in
`ob-get-entry-text'. Each hook is a function that could be called
with no parameter.

This is a good place for o-journal parser plugins."
  :group 'o-journal
  :type 'hook)
...
(defstruct (ob:blog :named)
 ...
 - template-dir: location of the template directory defined by
   ...
   directory of the o-journal library.
...
(defstruct (ob:post :named)
 ...
 - template: template to use for current post read from
   \"TEMPLATE\" property or \"journal_post.html\".
...
;;;###autoload
;;;###autoload
(defun o-journal-version (&optional here)
  "Message the current o-journal version. If call using
`universal-argument', insert value in current position."
  (interactive "P")
  (let* ((default-directory (file-name-directory (locate-library "o-journal")))
	 (has-git (and (file-exists-p (expand-file-name ".git"))
		       (executable-find "git")))
	 (version (if has-git
		      (substring (shell-command-to-string "git describe") 0 -1)
		    o-journal-version))
	 (msg (format "o-journal version %s" version)))
    (if here
	(insert msg)
      (message msg))))
...
(defun org-publish-blog-sync (file)
  ...
  (with-current-buffer (or
	...
	(funcall syncf (format "%s/%s"
      ...
      (run-hooks 'o-journal-after-publish-hook)
...
(defun o-journal-publish-linked-files()
	...
	(when ret
	  ...
	  (loop for f in ret
		do (let ((target (format "%s/%s/%s"
		     ...
		     (ob-do-copy f target))))))))
(add-hook 'o-journal-html-plugins-hook 'o-journal-publish-linked-files)
	...
	(let ((text (buffer-string)))
	  (with-temp-buffer
	    ...
	    (while (<= 2 (save-match-data (funcall outline-level)))
	      (org-promote-subtree))
	    (run-hooks 'o-journal-html-plugins-hook)
	    (goto-char (point-min))
...
(defun ob-write-index()
  "Publish all indexes (default, categories)"

  (ob-write-index-to-file "journal_archives.html"
			    (format "%s/journal.html"
				    (ob:blog-publish-dir BLOG)))

  (ob-write-index-to-file "journal_rss.html"
  			  (format "%s/index.xml"
  				  (ob:blog-publish-dir BLOG)))
  ...
  (loop for CATEGORY in (ob:get-posts nil nil nil 'category)
	with PATH-TO-ROOT = ".."
	do (unless (equal "." CATEGORY)
		 (ob-process-index "journal_index_category.html" CATEGORY))))
...
(defun ob-write-tags ()
  ...
  (let ((PATH-TO-ROOT ".."))
    (ob-write-index-to-file "journal_tags.html"
			    (format "%s/categories/index.html"
				    (ob:blog-publish-dir BLOG)))

    (loop for TAG in TAGS
	  do
	  (ob-write-index-to-file "journal_tags-details.html"
				  (format "%s/categories/%s.html"
					  (ob:blog-publish-dir BLOG)
					  (ob:tags-safe TAG))))))
